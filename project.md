...
| Sobre _(desativada por ora)_ | Trajetória, formação (UCL, Alemanha, HR), mentores, filosofia, valores — rota redireciona para a Home enquanto a seção "sobre (resumo)" da Home cobre o conteúdo. Estado atual, não definitivo: a página dedicada **deve** ser construída no futuro; reativar removendo o redirect em `next.config.ts` e descomentando o link de nav |
